...
class MatrixApp:
    def __init__(self, root):
        ...
        # Enable pruning
        enable_pruning_frame = tk.Frame(params_frame, bg="#f0f0f0")
        enable_pruning_frame.pack(fill="x", pady=(0, 10))
        self.enable_pruning_label = tk.Label(
            enable_pruning_frame, text="הפעל מסנן אפשרויות", bg="#f0f0f0", font=("Arial", 9)
        )
        self.enable_pruning_label.pack(anchor="w")
        self.enable_pruning_var = tk.BooleanVar(value=True)
        self.enable_pruning_checkbox = tk.Checkbutton(
            enable_pruning_frame, variable=self.enable_pruning_var, bg="#f0f0f0", font=("Arial", 9)
        )
        self.enable_pruning_checkbox.pack(anchor="w")
    ...
    def max_n_representative_intersection(
        groups, n, top_k, stats: Optional[dict], enable_pruning: bool
    ...
    def run(self):
        ...
        try:
            ...
            enable_pruning = self.enable_pruning_var.get()
            try:
            ...
            params = CalculationParams(
                n=n, mode=mode, relaxation=relaxation, top_k=top_k, enable_pruning=enable_pruning
            )
